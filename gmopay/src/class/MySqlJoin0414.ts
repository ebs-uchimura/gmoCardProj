...
class SQL {
  ...
  countJoinDB = async (args: countjoinargs): Promise<number> => {
    return new Promise(async (resolve) => {
      try {
        SQL.logger.debug('db: countjoinDB mode');
        // total
        let total: number;
        // query string
        let queryString: string;
        ...
        await this.doInquiry(queryString, placeholder)
          .then((result: any) => {
            ...
            if (result == 'error' || result == 'empty') {
              // initialize total
              total = 0;
              SQL.logger.debug(`countjoinDB: ${result}`);
            } else {
              // set total
              total = result[0]['COUNT(`' + table + '`.id)'];
              SQL.logger.debug('countjoinDB: success');
            }
          ...
          .catch((err: unknown) => {
            // error
            SQL.logger.debug('countjoinDB: error');
            console.log(err);
            resolve(0);
          });
      } catch (e: unknown) {
  ...
  selectJoinDB = async (args: joinargs): Promise<any> => {
    return new Promise(async (resolve) => {
      try {
        SQL.logger.debug('db: selectjoinDB mode');
        // query string
        let queryString: string;
        // placeholder
        let placeholder: any[];
        ...
        if (colLen > 0) {
          ...
          await this.doInquiry(queryString, placeholder)
            ...
            .catch((err: unknown) => {
              // error
              SQL.logger.debug('selectJoinDB: error');
              console.log(err);
              resolve('error');
            });
        }
  ...
  updateJoinDB = async (args: updatejoinargs): Promise<any> => {
    return new Promise(async (resolve1) => {
      try {
        SQL.logger.debug('db: updateJoinDB mode');
        // placeholder array
        let placeholder: any[] = [];
        // tmp placeholder array
        let tmpPlaceholder: any[] = [];
          ...
          promises.push(
            new Promise(async (resolve2) => {
              ...
              await this.doInquiry(queryString, placeholder)
                ...
                .catch((err: unknown) => {
                  // error
                  SQL.logger.debug('updateJoinDB: error');
                  console.log(err);
                  resolve2('error');
                });
            })
